...
export default async function fetchBankByBic(bic) {
  ...
  return {
    // 👇 ключ, который ждёт BankDetailsMain
    bank_name: bank.name?.payment || bank.name?.full || null,
    bic: bank.bic || bic,
    correspondent_account: bank.correspondent_account || null,
    address: bank.address?.value || null,
