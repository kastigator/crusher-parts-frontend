...
export default function UsedInTable({ partId }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      message.error("Не удалось загрузить список родителей")
      setRows([])
  ...
  const columns = useMemo(() => [
    { title: "Parent Cat #", dataIndex: "parent_cat_number", width: 180, render: (v) => <ValueDisplay value={v} /> },
    { title: "Родитель", dataIndex: "parent_description_ru",
      render: (_, r) => {
        const text = r.parent_description_ru || r.parent_description_en || null
        if (!text) return <ValueDisplay value={null} />
        return <Tooltip title={text}><span className="cell-ellipsis" style={{ maxWidth: 600 }}>{text}</span></Tooltip>
      } },
    { title: "Кол-во в родителе", dataIndex: "quantity", width: 180, align: "right", render: (v) => fmtQty(v) },
  ], [])

  if (!loading && rows && rows.length === 0) {
    return <div className="subtable-shell"><Empty description="Не используется ни в одной сборке" /></div>
  }
