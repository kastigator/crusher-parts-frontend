...
export default function BillingAddressPicker({ open, onClose, clientId, onPick }) {
  ...
  const load = async () => {
    ...
    try {
      ...
      const { data } = await axios.get("/client-billing-addresses", {
        params,
        signal: ctrl.signal,
      })
      ...
      if (name !== "AbortError" && name !== "ERR_CANCELED") {
        console.error(e)
        message.error("Не удалось загрузить юридические адреса")
      }
    } finally {
      setLoading(false)
    }
  ...
  return (
    <Drawer
      title="Выбрать юридический адрес"
      open={open}
      onClose={onClose}
      width={1000}
      destroyOnHidden
