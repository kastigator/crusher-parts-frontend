...
export default function PurchaseOrderPreviewPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError("")
      try {
        const { data } = await axios.get(`/purchase-orders/${id}/preview`, {
          responseType: "text",
          transformResponse: [(value) => value],
        })
        if (!cancelled) setHtml(typeof data === "string" ? data : "")
      } catch (e) {
        if (!cancelled) setError(e?.response?.data?.message || "Не удалось открыть предпросмотр заказа поставщику")
      } finally {
  ...
  return (
    <iframe
      title={`purchase-order-preview-${id}`}
      srcDoc={html}
